...
void Monster::update(std::vector<std::unique_ptr<Object>>& objects)
{
	...
	{
		std::cout << *this << " has died!!!" << std::endl << std::endl;
	}
}
...
Monster::Monster(const std::unique_ptr<Object>& player)
{
	...
	case Object::Type::dragon:
		strengthVariance = level * 4.0;
		healthVariance = (long long)level * level * 2.0;
		ACVariance = level * 3;
		break;
	}
...
void Monster::scare()
{
	if (!afraid)
	{
		std::bernoulli_distribution makeAfraid(.5);
		afraid = makeAfraid(engine);
	}
}
